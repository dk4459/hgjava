...
document.addEventListener('DOMContentLoaded', function(e) {
	...
	//등록 이벤트(click)
	document.getElementById('addBtn') //getElementById는 #이 필요없다
...
//한건생성
	function makeRow(obj = {}) {
		...
		btn.innerText = '삭제';
		//이벤트 연결
		let td = document.createElement('td'); //deleteRow()로 바꾸면, 클릭시 실행되는 의도와 달리 여기서 그냥 실행되어버림.
		td.appendChild(btn);
		tr.appendChild(td);
	...
	function deleteRow(e) {
	    ...
	    const rmvAjax = new XMLHttpRequest();
	    rmvAjax.open('post', 'removeBook.do');
	    rmvAjax.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
	    rmvAjax.send('bcode=' + bcode);
	    rmvAjax.onload = function (e) {
	            let result = JSON.parse(rmvAjax.responseText);
	            console.log(result);
	            console.log(document.querySelector('#' + 'book_' + bcode));
	            if (result.retCode == 'OK') {
	                let parent = document.querySelector('#show tbody');
	                let child = document.querySelector('#' + 'book_' + bcode);
